...
Scheduler::setStartTimeGait(trajectory_msgs::JointTrajectory trajectory,
                            ros::Time time) {
...
Scheduler::scheduleTrajectory(const march_shared_resources::GaitGoal *goal,
  ...
  if (!lastScheduledGaitInProgress()) {
    throw std::runtime_error(
        "There is already a gait scheduled in the future.");
  }
  ros::Time startingTime = getStartTime(offset);
  trajectory_msgs::JointTrajectory trajectory =
      setStartTimeGait(goal->current_subgait.trajectory, startingTime);
