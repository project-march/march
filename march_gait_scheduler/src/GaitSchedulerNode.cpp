...
void activeCallback() { ROS_DEBUG("Gait trajectory goal just went active"); }
...
void scheduleGaitCallback(
    const march_shared_resources::GaitGoalConstPtr &goal) {

  ROS_DEBUG("Gait Scheduler received msg to schedule: %s of %s",
  ...
  try {
    ...
    followJointTrajectoryAction->sendGoal(trajectoryMsg, &doneCallback,
                                          &activeCallback, &feedbackCallback);
...
int main(int argc, char **argv) {
  ...
  schedule_gait_action_server = new ServerFollowJoint(
      n, ActionNames::schedule_gait, &scheduleGaitCallback, false);
