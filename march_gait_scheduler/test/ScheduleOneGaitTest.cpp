...
protected:
  ...
  trajectory_msgs::JointTrajectory fake_sit_trajectory() {
    ...
    jointTrajectory.joint_names = {"left_hip",  "left_knee",  "left_ankle",
...
TEST_F(ScheduleOneGaitTest, ScheduleNow) {
  ...
  gaitGoal.name = "sit";
  const auto &gaitGoalConst =
      const_cast<const march_shared_resources::GaitGoal &>(gaitGoal);
  ...
  ASSERT_FLOAT_EQ(current_time.toSec(),
                  trajectoryMsg.goal.trajectory.header.stamp.toSec());
}
...
TEST_F(ScheduleOneGaitTest, ScheduleTrajectoryUnchanged) {
  ...
  for (int i = 0; i < trajectory.points.size(); i++) {
    ...
    trajectory_msgs::JointTrajectoryPoint point =
        trajectoryMsg.goal.trajectory.points[i];
    ASSERT_NEAR(pointExpected.time_from_start.toNSec(),
                point.time_from_start.toNSec(), 0.1);
    ASSERT_NEAR(pointExpected.accelerations.size(), point.accelerations.size(),
                0.1);
...
TEST_F(ScheduleOneGaitTest, ScheduleInTheFuture) {
  ...
  ASSERT_NEAR(futureTime.toSec(),
              trajectoryMsg.goal.trajectory.header.stamp.toSec(), 0.1);
  ASSERT_NE(currentTime.toSec(),
            trajectoryMsg.goal.trajectory.header.stamp.toSec());
}
