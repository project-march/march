...
TEST_F(ScheduleMultipleGaitsTest, ScheduleTwoNoOffset) {
  ...
  const auto &gaitSitGoalConst =
      const_cast<const march_shared_resources::GaitGoal &>(gaitSitGoal);

  march_shared_resources::GaitGoal gaitStandGoal = fake_stand_goal();
  const auto &gaitStandGoalConst =
      const_cast<const march_shared_resources::GaitGoal &>(gaitStandGoal);

  Scheduler scheduler;

  control_msgs::FollowJointTrajectoryActionGoal trajectoryMsgSit =
      scheduler.scheduleTrajectory(&gaitSitGoalConst,
                                   ros::Duration().fromSec(0));

  control_msgs::FollowJointTrajectoryActionGoal trajectoryMsgStand =
      scheduler.scheduleTrajectory(&gaitStandGoalConst,
                                   ros::Duration().fromSec(0));

  ros::Time secondStartTime = (current_time + gaitDuration);

  ASSERT_NEAR(current_time.toSec(),
              trajectoryMsgSit.goal.trajectory.header.stamp.toSec(), 0.1);
  ASSERT_NEAR(secondStartTime.toSec(),
              trajectoryMsgStand.goal.trajectory.header.stamp.toSec(), 0.1);
}
